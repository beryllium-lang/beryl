...
namespace beryl {
  ...
  public:
    Arena() noexcept : start(nullptr), size(0), offset(0) {}

    explicit Arena(size_t sz)
      : start(static_cast<std::byte*>(std::malloc(sz))),
      size(start ? sz : 0),
      offset(0) {
    }

    Arena(Arena&& other) noexcept
      : start(other.start), size(other.size), offset(other.offset) {
      other.start = nullptr;
      other.size = 0;
      other.offset = 0;
    }
    ...
    template <typename T>
    T* alloc(size_t count = 1) noexcept {
      if (count > 0 && count > std::numeric_limits<size_t>::max() / sizeof(T)) {
        return nullptr;
      }
      return static_cast<T*>(
        allocate_bytes(sizeof(T) * count, alignof(T)));
    }
